<script>
  ...
  const scope =
    "user-read-private user-read-email user-modify-playback-state user-read-playback-position user-library-read streaming user-read-playback-state user-read-recently-played playlist-read-private streaming";
  ...
  function callAuthorizationApi(body) {
    ...
    xhr.setRequestHeader(
      ...
      "Basic " +
        btoa(
          localStorage.getItem("client_id").toString() +
            ":" +
            localStorage.getItem("client_secret").toString()
        )
    );
...
<div>
  <button on:click={requestAuthorization} type="button"
    >1. Request Authorization</button
  >
  <button on:click={requestAccessToken} type="button"
    >2. Request Access Token</button
  >
</div>
